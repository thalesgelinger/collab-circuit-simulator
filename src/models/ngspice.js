export async function Spice(Module) {
  ...
  var out = Module["outrput"]?.push || console.log.bind(console);
  ...
  function preRun() {
    FS.writeFile("/test.cir", Module["netlist"]);
    if (Module["preRun"]) {
  ...
  const results = async () => {
    if (Module["output"][Module["output"].length - 1] === "DONE") {
